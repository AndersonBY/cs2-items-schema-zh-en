...
class ContainersCollector:
    ...
    def _find_item_indexes(self, item_name: str) -> Optional[str]:
        # performance? No, thanks
        matches = ITEM_NAME_RE.findall(item_name)
        if not matches:
            return None
    ...
    def _find_sticker_kit_index(self, item_name: str) -> Optional[str]:
        matches = ITEM_NAME_RE.findall(item_name)
        if not matches:
            return None
    ...
    def _find_music_kit_index(self, item_name: str) -> Optional[str]:
        matches = ITEM_NAME_RE.findall(item_name)
        if not matches:
            return None
    ...
    def __call__(self) -> tuple[dict[str, Any], ...]:
        ...
        for defindex, item_data in self.items_game["items"].items():
            ...
            if item_set_tag := item_data.get("tags", {}).get("ItemSet"):
                ...
                container["items"] = list(filter(None, (self._find_item_indexes(i) for i in loot_list)))
                ...
                loot_dict: Optional[dict[str, Any]] = None
                ...
                if sticker_caps_tag["tag_value"] in self.items_game["client_loot_lists"]:
                    ...
                    self.items_game["revolving_loot_lists"].get(item_data.get("attributes", {}).get("set supply crate series", {}).get("value"))
                    in self.items_game["client_loot_lists"]
                ):
                    loot_dict = self.items_game["client_loot_lists"][self.items_game["revolving_loot_lists"][item_data["attributes"]["set supply crate series"]["value"]]]

                if loot_dict is not None:
                    loot_list = self._get_loot_recursive(loot_dict)
                else:
                    loot_list = []
                container["kits"] = list(filter(None, (self._find_sticker_kit_index(i) for i in loot_list)))
                ...
                self.items_game["revolving_loot_lists"].get(item_data.get("attributes", {}).get("set supply crate series", {}).get("value")) in self.items_game["client_loot_lists"]
            ):
                loot_dict = self.items_game["client_loot_lists"][self.items_game["revolving_loot_lists"][item_data["attributes"]["set supply crate series"]["value"]]]
                ...
                if "musickit" in item_data["name"]:
                    container["musics"] = list(filter(None, (self._find_music_kit_index(i) for i in loot_list)))
